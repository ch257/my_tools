...
dofile(script_file_folder .. "modules\\lua\\common\\CSVFile_1_0.lua")
dofile(script_file_folder .. "modules\\lua\\common\\DataSetIterator_1_0.lua")
dofile(script_file_folder .. "modules\\lua\\common\\DataSetTools_1_0.lua")
...
function Template:main(arguments)
	...
	local rec = ds_tools:create_rec(data_set['columns'], input_file_format)
	
	ds_tools:add_cols({'<ZZ1>', '<ZZ2>'}, {'num', 'num'}, data_set)
	local add_cols_rec = ds_tools:create_rec({'<ZZ1>', '<ZZ2>'}, input_file_format)
	
	local ds_iterator = DataSetIterator:new(self.errors, data_set)
	while not ds_iterator.eods do
		ds_iterator:next_row(rec)
		
		if ds_iterator.row_count % 2 == 0 then
			add_cols_rec.set['<ZZ1>'](100 + ds_iterator.row_count)
			add_cols_rec.set['<ZZ2>'](nil)
		else
			add_cols_rec.set['<ZZ1>'](nil)
			add_cols_rec.set['<ZZ2>'](100 + ds_iterator.row_count)
		end
		
		local close = rec.get['<CLOSE>']()
		
		ds_tools:update_row(data_set, add_cols_rec, ds_iterator.row_count)
	end
	
	csv_file:print_data_set(data_set, output_file_format)
